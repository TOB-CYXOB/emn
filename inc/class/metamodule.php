...
';
    ...
    function __call($name, $params)
    {
        ...
        if ($config['metamodule_autocreate'] != 1 || // автодополнение отключено
            $name[0] == '_' ) // или первым символом идет "земля", т.е. это служебный метод
        { 
        ...
        else //добавляем этот метод в исходник модуля
        {
            ...
            $fname_tpl = $this->_tplDir(). $name . "_tpl.php";
...
// <= ТУТ рабочий код метода
        ...
        $html = $this->phptpl($page, $this->_tplDir()."'.$name.'_tpl.php");
...
';
	...
	function phptpl_include(&$page, $tplfile)    
	{
		global $_GTC;
		$OLD_GTC = clone($_GTC);
