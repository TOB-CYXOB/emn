...
class Sql
{
	...
	function connect($confArr=array(), $exit=true)	
    {
		...
		if (count($confArr))
		{
			...
			Sql::_log('open connection to '.$confArr['dbhost'].'...');
			...
			{
				Sql::_log('failed: '.$texts['sql_connection_error']);
			...
			Sql::_log($confArr['dbhost'].' connected');
			return true;
		}
		else //используем дефолтные настройки
		{
			...
			Sql::_log($config['dbhost'].' connected');
			return true;
		}
	...
	function connectToMaster()
	{
		...
		if (isset($this) && get_class($this) == "Sql")
		{
			if ($this->dbhost == $config['dbhost'])
			{
				...
				//Sql::_log('already Master DB');
				return;	
			}
			Sql::_log('reconnect to Master DB: '.$config['dbhost']);
