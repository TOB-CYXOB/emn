<?php
  class All {
	...
	//вернуть уникальный в рамках сайта ID
	function getID()
	{
		$q = sprintf("INSERT INTO prname_a_getrn SET stub=1;");
		sql::query($q);
		$ID = sql::insert_id();
		$q = sprintf("DELETE FROM prname_a_getrn WHERE rn=%d;", $ID);
		sql::query($q);
    ...
    function get_date($tmp_date, $param = 1)   
    {
		global $ar_mon;
    ...
    //---------------------  ПОЧТА  ---------------------------------
function send_mail($a_email, $email_sub, $email_text,$file='',$filename='')
{
	global $config;
...
$mailer->From = "robot@estmesto.net";
...
//---------------------------------------------------------------------------
	...
	function get_node($id, $depth=1000000, $type='full')		{
		global $control;
...
////////////////////////////////////////////////////////
   	function get_nav($id)
   	{
		...
		asort($all);
		if ($control->id){$r=$this->b_data_all($control->id);$all[]=$r;};
		return $all;		
	}
...
///СОРТИРОВКЕ ДА!!!!!!!!!
		...
		while ($arr = sql::fetch_assoc($res))
		{
			...
			for ($i=0;$i<count($qarr);$i++)
			{
				switch ($fields[$qarr[$i]]->datatkey)
		           	{
			    		case 'html':$page->$qarr[$i] = text_view($arr[$qarr[$i]]);break;
			    		case 'textarea':
    ...
    // variant1;variant2;variant3
			    		...
			    		break;
			    		case 'image':$page->$qarr[$i] = $arr[$qarr[$i]];
		                ...
		                break;
			    	    default:$page->$qarr[$i] = $arr[$qarr[$i]];break;
			    	}	
			}
		}
		return $page; 
...
function insert_block($templ,$parent,$arr,$visible=1)
{
  ...
  {
	if($qww['datatkey']=="date" && $arr[$qww['key']] == '') $arr[$qww['key']]=date("Y-m-d");
	if(strlen($arr[$qww['key']])==0)$arr[$qww['key']]=$qww['default'];
...
function c_data($catid, $templ,$relkey)
{
 ...
 function c_data_all($catid,$templ)
	{
