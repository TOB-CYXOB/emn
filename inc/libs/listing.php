...
class Listing	
{
    ...
    function get_item()
    {
		...
		while ($one_arr = sql::fetch_assoc($this->items))
			{  
				if($this->row && $tr==$this->row){$tr=0;$this->item[$i]->tr = true;}else{$tr++;$this->item[$i]->tr = false;};
				$ak = array_keys($one_arr);
				if(!$this->url[$one_arr['parent']] && !$this->is_global)$this->url[$one_arr['parent']] = $this->all->get_url($one_arr['parent']);
			    if ($this->type!== 'cats')$this->item[$i]->url = ($this->url[$one_arr['parent']]?$this->url[$one_arr['parent']]:$this->tmp_url) .$this->all->add_url(($this->nopage?'':$this->page), 'view', ($this->type=='c'?$one_arr['parent']:$one_arr['id']));
			    if ($this->type == 'cats')$this->item[$i]->url = $this->url[$one_arr['parent']];
			    for($ii=0;$ii<count($ak);$ii++)
			    {
			      switch ($this->fields[$ak[$ii]]->datatkey)
			    	{
			    		case 'html':$this->item[$i]->$ak[$ii] = text_view($one_arr[''.$ak[$ii].'']);break;
			    		case 'textarea':
                                {
    ...
    // variant1;variant2;variant3
                        ...
                        case 'double':
                                 ...
                                 $this->item[$i]->$ak[$ii] = $one_arr[$ak[$ii]];
                        break;
			    	    default:$this->item[$i]->$ak[$ii] = $one_arr[''.$ak[$ii].''];break;
			    	}
			    }
			$i++;
			}
